...
// Theme configuration
const createAppTheme = (mode: 'light' | 'dark', primaryColor: string, secondaryColor: string) =>
  createTheme({
    palette: {
      mode,
      primary: {
...
function AppContent() {
  const { state } = useApp();
  const theme = createAppTheme(state.theme.mode, state.theme.primaryColor, state.theme.secondaryColor);
